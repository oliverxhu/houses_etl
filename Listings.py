...
class Listings(ETLTools.ETLTools):
    ...
    def run_all_etl(self):
        ...
        for e in enumerate([self.agent_rental_error, self.rental_listings_error,
            ...
            if e:
                error_list.append(e)
        print(self.agency_flag)
        print(self.rental_listings_flag)
        print(self.agent_rental_flag)
        print(self.rental_photo_flag)
        print(error_list)
    ...
    def convert_ms_dates(self, df, table):
        ...
        for cols in date_cols:
            df[cols] = df[cols].apply(lambda x: x[x.find("(")+1:x.find(")")])
        return df
