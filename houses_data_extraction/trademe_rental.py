...
print('trademe_rental_hourly.py')
...
#base url
baseurl = 'https://api.trademe.co.nz/v1/Search/Property/Rental.json?photo_size=FullSize&page='
...
# file_path = '/home/ubuntu/projects/housing/data/rental/tm_rental_hourly_'+str(job_run_datetime).replace(':','_')+'.csv'
...
# s3.meta.client.upload_file(file_path, bucket_name , 'data/rental/tm_rental_hourly_'+str(job_run_datetime).replace(':','_')+'.csv')
...
if mode == 'testnodbwrite':
   print('mode = testnodbwrite')
    ...
    driver = 'postgresql'
    username = 'housingdataetl'
    host = 'localhost'
    #host = 'housing.ct0tluqftf3s.ap-southeast-2.rds.amazonaws.com'
    password = 'housingdata123'
    port = '5432'
    database = 'housing'
    schema = 'staging'
    table_name = 'tm_rental_listings_hourly'
    ...
    cur.execute("delete from %s.%s"%(schema,table_name))
    ...
    # cur.execute("select * from staging.rental_extract_id()")
    # print('executing staging.rental_extract_detail()')
    # cur.execute("select * from staging.rental_extract_detail()")
    # print('committing script')
    # conn.commit()
    # print('closing connection')
    # conn.close()
