...
class DatabaseConnection:
    ...
    def append_df_to_table(self, df, table, dtype_dict=None):
        ...
        :param types_dict: {column_name: SQLAlchemy_data_type} eg. from sqlalchemy.types import NVARCHAR
        :return: None
        """
        self._connection_test()
    ...
    def table_lookup(self, df, df_lookup_column_list, table, table_lookup_column_list, table_return_column_list,
                     return_suffix='_tbl', indicator=True):
        """
        ...
        :param return_suffix: suffix to add to lookup and returned columns
        ...
        return pd.merge(left=df, right=df_lookup, how='left', left_on=df_lookup_column_list,
                        right_on=table_lookup_column_list, suffixes=('', return_suffix), indicator=indicator)
    ...
    def add_new_records(self, df, df_lookup_column_list, table, table_lookup_column_list):
        ...
        merged_table = self.table_lookup(df=df,
                                         ...
                                         return_suffix='_tbl', indicator='record_exists')
